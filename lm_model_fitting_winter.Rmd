...
Our linear regression model for T2 is 
<pre><code>
T2  =  2.472731 - 0.063511 *  T2 (of the ECMWF model) + 0.564682 * D2 (of the ECMWF model) + 0.364282 * T_950  - 0.148566* T_950_M1 + 0.265225 *D2_M1 - 0.603928* TP +  0.035618 * RH_950 + 0.082966 * V10 - 0.629401* LCC
</code></pre>
...
## The diagnostic plots  Residuals vs Fitted values, Normal QQ-plot and Residuals vs Leverage for the models 

We plot the following graphs Residuals vs Fitted values, Normal QQ-plot and Residuals vs Leverage for all the models.
...
The third graph, Residuals vs Leverage graph helps to find the influential extreme values or outliers. The  data have extreme values,  but we can consider these extreme values are not influential if the linear  regression model we find does not change if we either include or exclude them from analysis. 
...
par(mfrow = c(2,2))
plot(my.model.T2, which = c(1,2,5), caption = list("T2-my model  Residuals vs Fitted values", "T2-mymodel Normal QQ-plot", "T2-mymodel Residuals vs Leverage"))
