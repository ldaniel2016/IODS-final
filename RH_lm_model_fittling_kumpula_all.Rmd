...
The linear regression model takes two response variables namely T2Obs and D2Obs, the observations of air temperature and dewpoint temperature at 2 meters. As the predictors we use the ECMWF model outputs. All these variables are explained in the Section on weather data. Using the linear regression model, we predict the
T2 and D2, the air temperature and dewpoint temperature for 64 forecast periods. We then calculate the relative humidity, RH, using the predicted T2 and D2 using the three linear regression models. We also calculate the RH using the T2 and D2 from the ECMWF raw model. The relative humidity is calculated using some standard formulas.
...
ModelsOfAlgorithms <- function(training.set, test.set) {
...
CalculateRH <- function(wdata) {
  ...
  # Models and predictions for T2 and D2
  
  results.T2 <- ModelsOfAlgorithms(T2_Obs_model_data_train, T2_Obs_model_data_test)
  results.D2 <- ModelsOfAlgorithms(D2_Obs_model_data_train, D2_Obs_model_data_test)
  ...
  # T2 and D2 from predictions using lm, lmstep and glmcv
  ...
  # rmse for T2 and  D2 obtained using the rawmodel,lm, lmstep and glmcv  models
  ...
  # predicted T2, D2 using lm, lmstep and glmcv
...
df_winter_T2_pred_model <- df_winter_pred_model[, c(4:8)]
df_winter_D2_pred_model <- df_winter_pred_model[, c(4,9:12)]
...
df_winter_rh_model <- df_winter_rh_model[, c(4:8)]
...
df.rmse.pred.station.fperiod <- df.rmse.pred[,c(2, 4:8)]
...
df.rmse.pred.station.fperiod <- df.rmse.pred[,c(2, 4,9:12)]
...
df.rmse.rh.station.fperiod <- df.rmse.rh[,c(2, 4:8)]
